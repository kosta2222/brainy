            ...
            for row in range(layer.out):
                if layer.with_bias:
                    if elem == 0:
                        ...
                        tmp_v += layer.matrix[row][elem] * inputs[row]
                else:
                    tmp_v += layer.matrix[row][elem] * inputs[row]

            layer.cost_signals[elem] = tmp_v
...
def feed_forwarding_back(nn_params: Nn_params, inputs, loger):
    last_layer_ind = nn_params.nl_count - 1
    make_hidden_back(nn_params, last_layer_ind, inputs, loger)
    j = last_layer_ind - 1
    for i in range(last_layer_ind, -1, -1):
        inputs = get_hidden(nn_params.net[i + 1])
        make_hidden_back(nn_params, i, inputs, loger)

    first_layer = nn_params.net[0]

    # Для вывода сократим если нужно массив
    out_l = [0]*first_layer.in_
    first_layer_hidden = get_hidden(first_layer)
    for elem in range(len(out_l)):
        out_l[elem] = first_layer_hidden[elem]
    return out_l
